...
4. Wire it as `<service>:<service>-oidc-bootstrap.sh` in the `POST_START_HOOKS` list of `scripts/stack-up.sh` — the sequence the systemd unit and `make update` both run (the `<service>:` prefix gates it on `COMPOSE_PROFILES`; drop it for a core service). No systemd change is needed; `tests/stack-up-test.sh` fails if a hook script exists but nothing runs it. `tests/compose-test.sh` covers the compose side: a `depends_on: service_healthy` whose target declares no `healthcheck`, a moving image tag, a router exposed on `websecure` with no middlewares, a service depending on `postgres` with no role in `config/postgres/init-databases.sh`, and a resource key the kernel never reads — a `deploy:` block (compose drops it outside swarm), a missing `mem_limit`, a `mem_reservation` as large as the limit above it, or a `memswap_limit` that silently disables swap.
