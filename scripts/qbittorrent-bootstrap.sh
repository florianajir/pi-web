...
# The categories Prowlarr routes releases into, "<name>:<save path>". Each save path
# is a folder compose mounts into Kavita read-only, so this list must stay in step
# with the kavita volumes in compose.yaml and with scripts/kavita-pre-start.sh.
# Everything else keeps the client default ("prowlarr"), which Kavita never reads.
LIBRARY_CATEGORIES="manga:/downloads/manga books:/downloads/books"
