...
get_beszel_oidc_client_secret() {
    local explicit_secret data_root secret_file secret_value

    explicit_secret=$(get_env_value BESZEL_OIDC_CLIENT_SECRET)
    ...
    secret_file="$data_root/authelia-config/secrets/oidc_beszel_secret.txt"
    ...
    # Fallback for root-owned secrets generated by systemd pre-start hooks.
    secret_value=$(docker compose exec -T authelia sh -ec 'cat /config/secrets/oidc_beszel_secret.txt' 2>/dev/null | tr -d '\r\n')
...
get_beszel_runtime_env_value() {
    # Read env vars from running beszel container.
    local key="$1"
...
prepare_agent_env_for_passwordless_mode() {
    # In OIDC-only mode, password login is unavailable. Reuse existing TOKEN/KEY.
...
configure_beszel_oidc_provider() {
    ...
    client_secret=$(get_beszel_oidc_client_secret) || {
...
main() {
    ...
    if beszel_password_auth_disabled; then
        ...
        if [ -z "$UNIVERSAL_TOKEN" ] || [ -z "$HUB_PUBLIC_KEY" ]; then
            log "ERROR: Failed to bootstrap TOKEN/KEY using superuser fallback"
            exit 1
        fi
