...
# True only when $CONFIG already carries a usable login, i.e. auth is not
# disabled AND at least one user is defined. Anything else - no file, no auth
# key, "disabled": true, an empty users list - counts as unprotected.
auth_already_configured() {
  [ -f "$CONFIG" ] || return 1
  jq -e '(.auth.disabled // false) == false and ((.auth.users // []) | length) > 0' \
    "$CONFIG" >/dev/null 2>&1
}

bootstrap_auth() {
  # Fail closed on a missing credential. BACKREST_AUTH_PASSWORD comes from
  # config/backrest/backrest.env, generated by scripts/backrest-pre-start.sh and
  # loaded through an `env_file: required: false` - so a `docker compose up` that
  # skipped the pre-start hook arrives here with it unset. Returning early then
  # would leave config.json.template's `"auth": {"disabled": true}` in place and
  # serve the restic password and the S3 keys to any caller on the network. That
  # could not happen while the value was ${PASSWORD}, which is never empty.
  #
  # Refusing to start is the right end: backrest is a backup daemon, so a loud
  # stop costs one missed snapshot, while starting unprotected costs the archive.
  if [ -z "${BACKREST_AUTH_USER:-}" ] || [ -z "${BACKREST_AUTH_PASSWORD:-}" ]; then
    if auth_already_configured; then
      log "BACKREST_AUTH_USER/PASSWORD unset; keeping the login already in $CONFIG"
      return 0
    fi
    log "FATAL: BACKREST_AUTH_USER/BACKREST_AUTH_PASSWORD are unset and $CONFIG"
    log "       defines no login, so :9898 would hand the restic password and the"
    log "       S3 keys to any caller. Run scripts/backrest-pre-start.sh (or"
    log "       \`make start\`, which does) to generate config/backrest/backrest.env."
    exit 1
  fi
