...
**The browser downloads a 9-byte `Forbidden` file instead of showing the 403.** Same cause, different
symptom, and it means the `autodetect` middleware is not reaching that response — the denial carries no
`Content-Type`, and `nosniff` leaves the browser nothing to render. Check that `autodetect@docker` is
still last on the `websecure` entrypoint chain ([Security](SECURITY.md#the-middleware-chain)); listed
anywhere but innermost, it no longer wraps the middleware that wrote the response.
