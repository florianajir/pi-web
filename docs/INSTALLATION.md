...
4. Asks which optional services to enable, using the very same picker as `make config` — everything pre-selected, grouped into sections, linked services toggling together; core infrastructure (Traefik, Authelia, Pi-hole, Headscale, ...) always runs, and the choice can be changed anytime (see [Choosing which services run](CONFIGURATION.md#choosing-which-services-run))
5. Runs `make preflight` then `make install`

Prompts use `whiptail` dialogs when it is installed (it ships with Raspberry Pi OS); otherwise the installer falls back to plain terminal prompts, with identical behavior. The service picker is the repository's own `scripts/services-picker.py`, so there is only one screen to learn and it cannot drift from `make config`; a host without `python3` skips that step and keeps every service enabled.
